...
encoder_input="input_overlap.in"       # Encoder Sentences
decoder_input="label_overlap.out"      # Decoder Sentences
# decoder_input="label.out"      # Decoder Sentences

outputDir="activationsOutput_no_filtering/"    # Specify the output directory for the extractions
...
sentence_length=3000   # maximum sentence length
minfreq=0 
maxfreq=15
delfreq=10000000 
layers_of_interest="0,1,3,6,9,12" # define layers of interest 
layers_of_interest="1" # define layers of interest 
...
PYTHONPATH=$NEUROX_PATH python3 -u NeuroX/neurox/data/extraction/transformers_extractor.py "${model},${model},${model_class}" ${encoder_working_file}  ${decoder_working_file} "activations.json" --aggregation "average"  --output_type json --seq2seq_component both --decompose_layers --filter_layers "$layers_of_interest"
