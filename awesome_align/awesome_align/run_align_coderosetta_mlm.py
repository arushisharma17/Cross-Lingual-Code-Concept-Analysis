...
class LineByLineTextDataset(IterableDataset):
    ...
    def process_line(self, worker_id, line):
        ...
        # Use the tokenizer directly with truncation and padding
        ...
        bpe2word_map_src = src_encoding.word_ids()
        bpe2word_map_tgt = tgt_encoding.word_ids()
        
        # Remove None values and convert to list of indices
...
def main():
    ...
    # Load CodeRosetta model and tokenizer
    if args.tokenizer_name:
        ...
        raise ValueError(
            "You must specify either tokenizer_name or model_name_or_path"
        )

    # Update modeling constants with CodeRosetta tokenizer values
    modeling.PAD_ID = tokenizer.pad_token_id
    modeling.CLS_ID = tokenizer.bos_token_id  # CodeRosetta uses bos instead of cls
    modeling.SEP_ID = tokenizer.eos_token_id  # CodeRosetta uses eos instead of sep
