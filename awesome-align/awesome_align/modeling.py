...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def get_aligned_word(self, ids_src, ids_tgt, bpe2word_map_src, bpe2word_map_tgt, device, 
                        src_start_idx=0, tgt_start_idx=0, align_layer=8, extraction='softmax', 
                        softmax_threshold=0.001, test=False, output_prob=False):
        batch_size = ids_src.size(0)
        assert batch_size == 1, "Currently only support batch_size=1"
        
        with torch.no_grad():
            outputs_src = self(ids_src.to(device), output_hidden_states=True)
            outputs_tgt = self(ids_tgt.to(device), output_hidden_states=True)

            hidden_states_src = outputs_src.hidden_states[align_layer]
            hidden_states_tgt = outputs_tgt.hidden_states[align_layer]

            attention_probs = torch.matmul(hidden_states_src[0], hidden_states_tgt[0].transpose(-1, -2))
            
            if extraction == 'softmax':
                attention_probs = torch.nn.Softmax(dim=-1)(attention_probs)
            elif extraction == 'entmax':
                attention_probs = entmax15(attention_probs, dim=-1)
            
            word_aligns = []
            for i in range(len(bpe2word_map_src)):
                if bpe2word_map_src[i] == -1:
                    continue
                for j in range(len(bpe2word_map_tgt)):
                    if bpe2word_map_tgt[j] == -1:
                        continue
                    if attention_probs[i,j] > softmax_threshold:
                        word_aligns.append((bpe2word_map_src[i], bpe2word_map_tgt[j], attention_probs[i,j]))
            
            return word_aligns

def init_codebert_weights(model):
    """Initialize CodeBERT specific weights"""
    if isinstance(model, (BertForMaskedLM, BertModel)):
        # Initialize the model weights with CodeBERT's initialization scheme
        model.apply(model._init_weights)
        # Ensure embedding weights are properly initialized
        model.bert.embeddings.position_embeddings.weight.data.normal_(mean=0.0, std=model.config.initializer_range)
