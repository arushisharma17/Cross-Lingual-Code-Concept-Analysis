...
def label_clusters(clusters_data):
    ...
    for index, (cluster_id, cluster_info) in enumerate(clusters_data.items()):
        ...
        user_message = f"""
        You are analyzing a cluster of Java tokens and their context sentences. Each cluster has one or more unique tokens. Your task is to identify the syntactic role or syntactic function these tokens play within the context of the provided sentences. Also focus on the semantic significance of the tokens and what is being achieved in the code.

        Guidelines for Analysis:
        1. **Tokens**: Review the provided tokens. Consider their role in Java programming (e.g., keywords, operators, identifiers,etc).
